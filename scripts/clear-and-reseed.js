// Clear and reseed with correct data structure
...
async function clearListings() {
  ...
  try {
    ...
    console.log(`  ✅ Cleared ${count} listings`);
  } catch (error) {
    console.error('  ❌ Failed to clear listings:', error.message);
  }
}

// Create test listings with correct structure
async function createListings(count = 30) {
  console.log(`Creating ${count} test listings with correct structure...`);
  let successCount = 0;
  let errorCount = 0;
...
// Main function
async function main() {
  console.log('🌱 Starting database clear and reseed...\n');
  ...
  try {
    ...
    // Create new listings with correct structure
    ...
    if (result.successCount > 0) {
      console.log('\n🎉 Listings should now appear in your browse page!');
      console.log('   The data structure now matches the query expectations:');
      console.log('   - status: "available" (not "active")');
      console.log('   - location.city (not address.city)');
    }
